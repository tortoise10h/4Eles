       ...
       console.log("before event listener: " + urlArr);
        $("#searchInput").on('keyup keypress',function(e){
            ...
            if(keyCode == 13){
                ...
                let subLink = 'http://localhost:8080/web2/searches/index?search=' + inputValue + "&page=" + urlArr[2];
        ...
        function loadProducts(searchValue = '', page = 1){
            $.ajax({
                url: 'http://localhost:8080/web2/searches/getSearchProducts?search=' + searchValue + "&page=" + page,
                type: 'POST',
                cache: false,
                // data: {page:page}, 
            ...
            $.each(products,function(tag,product){
                text += '<div class="img-card col-sm-6 col-lg-4 mb-4" data-aos="fade-up">' +
                            ...
                            '<figure class="block-4-image">' + 
                                '<a href="http://localhost:8080/web2/shops/productDetail/' + product.id.substr(0,3) + '/' + product.id +'"><img src="http://localhost:8080/web2' + product.imgLink + '/' + product.id + '_1.jpg"' + 'class="img-fluid"></a>' +
                            '</figure>' +
                            '<div class="img-card-name block-4-text p-4">' +
                                '<h3><a href="http://localhost:8080/web2/shops/productDetail/' + product.id.substr(0,3) + '/' + product.id +'">'+ product.name + '</a></h3>' +
                            '</div>'+
        ...
        function createPaginationShow(totalPages,pageActive,searchValue){
            ...
            let text = '<ul><li><button data="http://localhost:8080/web2/searches/getSearchProducts?search=' + searchValue + '&page=' + (active - 1) + '" class="pagination-btn" id="' + (active - 1) + '">&lt;</button></li>';  //button pre
        
            for(let i = 1; i <= totalPages; i++){
                if(pageActive == i){
                    text += '<li><button data="http://localhost:8080/web2/searches/getSearchProducts?search=' + searchValue + '&page=' + i + '" class="pagination-btn pagination-active" id="'+ i +'">'+ i + '</button></li>';
                }else{
                    text += '<li><button data="http://localhost:8080/web2/searches/getSearchProducts?search=' + searchValue + '&page=' + i + '" class="pagination-btn" id="'+ i +'">'+ i + '</button></li>';
                }
            }
            text += '<li><button data="http://localhost:8080/web2/searches/getSearchProducts?search=' + searchValue + '&page=' + (active + 1) + '" class="pagination-btn" id="' + (active + 1) + '">&gt;</button></li></ul>'; //button next
            return text;
        }
